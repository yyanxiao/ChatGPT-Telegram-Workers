...
import { createTelegramBotAPI } from '@chatgpt-telegram-workers/telegram';

export async function loadChatRoleWithContext(
    chatId: number,
    speakerId: number,
    ...
    if (groupAdmin === null || !Array.isArray(groupAdmin) || groupAdmin.length === 0) {
        ...
        // 缓存120s
        await ENV.DATABASE.put(groupAdminsKey, JSON.stringify(groupAdmin), { expiration: Date.now() / 1000 + 120 });
    }
